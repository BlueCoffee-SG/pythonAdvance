...
class Node(db.Model):
    ...
    state=db.Column(db.Integer,nullable=True)  #节点是否禁用
...
####
# 限制传入的类型和范围（整数，且满足1-65）
class Age(object):
    def __init__(self,default_age=19) -> None:
    ...
    def __set__(self,isinstance,value):
        if value not in self.age_range:
            raise ValueError('must be in (18-66)')
